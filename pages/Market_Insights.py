...
lowest_price = (
    filtered_df["price_usd"].min()
)

median_price = (
    filtered_df["price_usd"].median()
)

highest_price = (
    filtered_df["price_usd"].max()
)

median_size = (
    filtered_df["size_m2"].median()
)
    ...
    [
        {
            ...
            "value": f"${highest_price:,.0f}",
            "icon": "📈",
        },
        {
            "label": "Median Size",
            "value": f"{median_size:,.0f} m²",
            "icon": "📐",
        },
        {
            "label": "Districts Covered",
            "value": str(district_count),
            "icon": "📍",
        },
    ],
    columns=st.columns(
...
st.caption(
    "Lowest and highest values show the advertised "
    "asking-price range among listings matching "
    "the current filters."
)
...
section_header(
    "Most Common Price Range",
    subtitle=(
        "Shows the asking-price range with the most "
        "listings for each selected property type."
    ),
)
...
if not common_range_results:
    ...
    st.info(
        "No listings are available for this "
        "section with the current filters."
    )
        ...
        with column:
            ...
            with st.container(
                ...
                if (
                    ...
                    st.warning(
                        "Very few listings match "
                        "the current filters. "
                        "This range is descriptive "
                        "only and should not be "
                        "treated as a market trend."
                    )
...
st.info(
    "The most common price range shows where "
    "advertised listings are most concentrated. "
    "It does not represent an official "
    "property valuation."
)
...
section_header(
    ...
    subtitle=(
        ...
        "Only districts with at least 20 matching "
        "listings are shown."
    ),
)
...
if district_price_chart_df.empty:
    ...
    st.info(
        "There are not enough listings for "
        "a reliable district asking-price "
        "comparison with the current filters."
    )
...
section_header(
    "District Details",
    subtitle=(
        "Detailed statistics for districts "
...
if property_type_filter == "All":
    ...
    section_header(
        "Condo vs Penthouse",
        subtitle=(
            "Compare typical advertised prices, "
    ...
    section_header(
        ...
        subtitle=(
            f"Typical advertised pricing and "
            f"characteristics for "
            f"{property_type_filter} listings."
        ),
    )
...
if (
    len(filtered_df)
    >= 5
    and not property_type_summary.empty
):
...
if (
    ...
    st.info(
        "Penthouses typically have higher asking "
        "prices and larger property sizes than Condos. "
        "They also tend to have a higher advertised "
...
section_header(
    "Bedroom & Bathroom Analysis",
    subtitle=(
        "Explore how advertised asking prices vary "
        "with the number of bedrooms and bathrooms."
    ),
)
...
if total_filtered_listings < 5:
    ...
    st.info(
        "These characteristics describe only "
        "the small number of listings matching "
        "the filters and should not be interpreted "
        "as a wider market trend."
    )
    ...
    if (
        ...
        st.warning(
            f"Only "
            f"{total_filtered_listings} "
            "listings match the current filters. "
            "Results are descriptive and may "
            "not represent the wider market."
        )
    ...
    with bed_col:
        ...
        if bedroom_summary.empty:
            ...
            st.info(
                "Bedroom information is not "
                "available for enough of the "
                "matching listings."
            )
    ...
    st.info(
        ...
        "However, room count is only one factor — "
        "property size, location, floor level, and "
...
disclaimer(
    "Market Insights are based on advertised "
    "property listing data collected for the "
    "PP PropertyLens project. They describe "
    "listing patterns and should not be "
    "interpreted as official transaction-price "
    "statistics."
)